...
class PGLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, new_rewards=None, redist_steps=None):
        ...
        vals, critic_train_stats = self._train_critic(batch, rewards, terminated, actions, avail_actions,
        ...
        baseline = vals.squeeze(-1)

        # Calculate policy grad with mask
        # th.cat([th.zeros_like(vals), th.zeros_like(vals)[:, -1:]], 1).squeeze(3)
        ret = build_td_lambda_targets(rewards, terminated, critic_mask, th.zeros_like(th.cat([rewards, rewards[:, -1:]], 1)), self.n_agents, self.args.gamma,
                                td_lambda=1.0)
    ...
    def _train_critic(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t):
        # Optimise critic
        target_vals = self.target_critic(batch)[:, :]
        ...
        vals = th.zeros_like(target_vals)[:, :-1]
