...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, train=False, dropout=0.2):
        dk = query.size()[-1]
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, train=False):
        ...
        h = F.dropout(self.lnorm_1(h + v), 0.2, train)
        h_out = torch.tanh(self.linear_2(h))
        h_out = F.dropout(self.lnorm_2(h_out + h), 0.2, train)
